...
import BottomSheetModalShow from "../../Pages/Home/BottomSheetModalShow";
...
const NearestPlacHomeCard = ({ item }: any) => {
  ...
  useEffect(() => {
    const loadIcon = async () => {
      try {
        const icon = await createCustomMarkerIcon(item.photos?.[0] || "");
    ...
    <div className="bg-theme-secondary rounded-2xl overflow-hidden">
      <div className="flex flex-col">
        <div className="mt-3  gap-2">
          <span className="text-black place-name">{item.name}</span>
          <span className="distance ml-2">({item?.distance}km away)</span>
        </div>
        ...
        <div className="px-4 mt-2">
          <LocationDescription description={item?.description} />
        </div>
        ...
        <div>
          <button
            className="check-btn  mt-5 mb-20"
            onClick={() => {
